...
| Name             | Type    | Description                        |
|------------------|---------|------------------------------------|
| `aws_ami_id` | String | AWS AMI ID. Will default to the latest Ubuntu 22.04 server image (HVM). Accepts `ami-###` values. |
| `ec2_instance_profile` | String | The AWS AMI instance profile to use for the EC2 instance. Default is `${GITHUB_ORG_NAME}-${GITHUB_REPO_NAME}-${GITHUB_BRANCH_NAME}`|
| `ec2_instance_type` | String | The AWS AMI instance type to use. Default is `t2.small`. See [this list](https://aws.amazon.com/ec2/instance-types/) for reference. |
| `ec2_volume_size` | Integer | The size of the volume (in GB) on the AWS Instance. | 
| `create_keypair_sm_entry` | Boolean | Generates and manages a secret manager entry that contains the public and private keys created for the ec2 instance. |
<hr/>
<br/>
...
| Name             | Type    | Description                        |
|------------------|---------|------------------------------------|
| `aws_create_efs` | Boolean | Toggle to indicate whether to create an EFS and mount it to the ec2 as a part of the provisioning. Note: The stack will manage the EFS and will be destroyed along with the stack |
| `aws_create_ha_efs` | Boolean | Toggle to indicate whether the EFS resource should be highly available (target mounts in all available zones within a region) |
| `aws_create_efs_replica` | Boolean | Toggle whether a read-only replica should be created for the EFS primary file system |
| `aws_enable_efs_backup_policy` | Boolean | Toggle whether the EFS should have a backup policy |
...
| `aws_postgres_database_final_snapshot` | Boolean | Creates a snapshot before deletion. If a string is passed, it will be used as the snapshot name. Defaults to `false`.|
<hr/>
<br/>
...
| `create_root_cert` | Boolean | Generates and manages the root cert for the application. **See note**. Default is `false`. |
| `create_sub_cert` | Boolean | Generates and manages the sub-domain certificate for the application. **See note**. Default is `false`. |
...
| `app_directory` | String | Relative path for the directory of the app. (i.e. where the `docker-compose.yaml` file is located). This is the directory that is copied into the EC2 instance. Default is `/`, the root of the repository. Add a `.gha-ignore` file with a list of files to be excluded. (Using glob patterns). |
...
| `app_port` | String | Port to be exposed for the container. Default is `3000` | 
<hr/>
<br/>
...
| `ansible_start_docker_timeout` | String | Amount of time in seconds it takes Ansible to mark as failed the startup of docker. Defaults to `300`.|
<hr/>
<br/>
<br/>
...
Most resources will contain the tag `${GITHUB_ORG_NAME}-${GITHUB_REPO_NAME}-${GITHUB_BRANCH_NAME}`, and some of them, even the resource name after. 
We limit this to a 60-character string because some AWS resources have a length limit; we shorten it if needed.

We use the Kubernetes style for this. For example, `kubernetes` -> k(# of characters)s -> k8s. And so you might see some compressions are made.

For some specific resources, we have a 32-character limit. If the identifier length exceeds this number after compression, we remove the middle part and replace it with a hash of the string itself. 
...
If `domain_name` is defined, we will look up a certificate with the name of that domain (eg. `example.com`). We expect that certificate to contain both `example.com` and `*.example.com`. 
...
To change a certificate (root_cert, sub_cert, ARN, or pre-existing root cert), you must first set the `no_cert` flag to true, run the action, then set the `no_cert` flag to false, add the desired settings and execute the action again. (**This will destroy the first certificate.**)
...
Users looking to add non-ephemeral storage to their created EC2 instance have the following options: create a new EFS as a part of the EC2 deployment stack, or mount an existing EFS. 

### 1. Create EFS

Option 1, you have access to the `create_efs` attribute which will create an EFS resource and mount it to the EC2 instance in the application directory at the path: `app_root/data`.
